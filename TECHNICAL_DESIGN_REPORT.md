...
For each data point $x_i$ on manifold $\mathcal{M}$:

1. **Find neighbors**: Let $\mathcal{G}_i = \{x_{i_k}\}_{k=1}^{N_i}$ be the set of $N_i$ neighboring points selected by shortest Euclidean distances.

2. **Create local data matrix**:
$$M_i = [x_{i_1} - \mu_i \quad \cdots \quad x_{i_{N_i}} - \mu_i]$$
where $\mu_i$ is the mean of neighboring points.

3. **SVD decomposition**:
$$M_i = U_i \Sigma_i V_i^T$$
where $U_i = [u_1 \cdots u_{N_i}]$.

4. **Define local subspace**:
$$\mathcal{S}_i = \text{span}\{u_1, \ldots, u_{d_i}\}$$
...
**Definition 1 (Weighted Angle):** Let $M_i$ and $M_j$ be matrices whose columns are data points for subspaces $\mathcal{S}_i$ and $\mathcal{S}_j$. With SVDs $M_i = U_i\Sigma_i V_i^T$ and $M_j = U_j\Sigma_j V_j^T$, define:

$$Q = (U_i\Sigma_i)^T(U_j\Sigma_j)$$

Compute SVD of $Q$: $Q = U_Q\Sigma_Q V_Q^T$

The **weighted angle** between $\mathcal{S}_i$ and $\mathcal{S}_j$ is:

$$\boxed{\theta_{ij} = \arccos\left(\frac{\text{tr}(\Sigma_Q)}{\text{tr}(\Sigma_i^T\Sigma_j)}\right)}$$

Note: If $\Sigma_i^T\Sigma_j$ or $\Sigma_Q$ is not square, zero-pad to make square.

#### Point-Wise and Manifold Curvature

**Definition 2 (Curvature):** For manifold $\mathcal{M}$ with data points $\mathcal{C} = \{x_i\}_{i=1}^N$:

1. **Local curvature at $x_i$**: Average weighted angle between $\mathcal{S}_i$ and its neighboring subspaces:
$$\theta_i = \frac{\sum_{j=1}^{N_i} \theta_{ij}}{N_i}$$

2. **Manifold curvature**:
$$\boxed{\rho_{\mathcal{M}} = \frac{\sum_{i=1}^N \theta_i}{N}}$$

#### Robustness Measure

**Definition 3 (Robustness):** For network $\text{Net}_k$ with $m$ output class manifolds $\{\mathcal{M}_i^k\}_{i=1}^m$:

$$\boxed{R_{\text{Net}_k} = \frac{\sum_{i=1}^m \rho_{\mathcal{M}_i^k}}{m}}$$
...
Assume data $Y = \{y_i\}_{i=1}^N \subset \mathbb{R}^D$ lies on a $d$-dimensional manifold $\mathcal{M}$. The embedding map $f: \mathbb{R}^d \to \mathbb{R}^D$ satisfies:

$$y_i = f(x_i) + \epsilon_i$$

where $X = \{x_i\}_{i=1}^N \subset \mathbb{R}^d$ are low-dimensional representations.

#### Local Coordinate Frame

For point $y_i \in Y$ with neighborhood $\{y_{i_1}, \ldots, y_{i_K}\}$:

1. Use SVD to construct **local natural orthonormal coordinate frame**:
$$\left\{\frac{\partial}{\partial x^1}, \ldots, \frac{\partial}{\partial x^d}, \frac{\partial}{\partial y^1}, \ldots, \frac{\partial}{\partial y^{D-d}}\right\}$$

   - First $d$ vectors: tangent space basis
   - Remaining $D-d$ vectors: normal space basis

2. In this frame, embedding becomes:
$$f(x^1, \ldots, x^d) = [x^1, \ldots, x^d, f^1, \ldots, f^{D-d}]$$

#### Hessian Estimation via Taylor Expansion

For each component $f^\alpha$ ($\alpha = 1, \ldots, D-d$), second-order Taylor expansion at $u_{i_j}$ around $x_i$:

$$f^\alpha(u_{i_j}) \approx f^\alpha(x_i) + \Delta_{x_i}^T \nabla f^\alpha + \frac{1}{2}\Delta_{x_i}^T H^\alpha \Delta_{x_i}$$

where:
- $\Delta_{x_i} = u_{i_j} - x_i$ (projected neighbor displacement)
- $H^\alpha = \frac{\partial^2 f^\alpha}{\partial x^i \partial x^j}$ is the Hessian matrix

This forms a **linear regression problem** to estimate $H^\alpha$ from the neighborhood.

#### Principal Curvatures

**Principal curvatures** are the eigenvalues of $H^\alpha$:
$$\text{eigenvalues}(H^\alpha) = \{\kappa_1^\alpha, \ldots, \kappa_d^\alpha\}$$

Each sample has $(D-d) \times d$ principal curvatures total.

#### Mean Absolute Principal Curvature (MAPC)

$$\boxed{\text{MAPC} = \frac{1}{N(D-d)d}\sum_{i=1}^N \sum_{\alpha=1}^{D-d}\sum_{k=1}^d |\kappa_k^\alpha(y_i)|}$$
...
| **Primary metric** | $\rho_{\mathcal{M}}$ (average weighted angle) | MAPC (mean absolute principal curvature) |
...
| Black-box robustness measure | Average curvature → robustness proxy | $R_{\text{curv}}(t)$ for comparison |

### New Measurements We Add

| Our Measurement | Definition | Purpose |
|----------------|------------|---------|
| Layer-wise dissipation $D_l(t)$ | $\|\nabla_{W_l}\mathcal{L}(t)\|_F^2$ | Optimization activity indicator |
| Parameter motion $\Delta W_l(t)$ | $W_l(t+1) - W_l(t)$ | Actual displacement vs. gradient |
| Temporal evolution $K_l(t)$ | Curvature at each checkpoint | Track geometric reorganization |
| Lagged correlation | $D_l(t) \to K_l(t+\tau)$ | Test predictive relationship |
...
| Weighted angle $\theta_{ij}$ | Same | Equation (2) from Sekmen-Bilgin |
| Manifold curvature $\rho_{\mathcal{M}}$ | $K_l(t)$ | Per-layer, per-checkpoint |
| Robustness $R_{\text{Net}_k}$ | $R_{\text{curv}}(t)$ | Average over output manifolds |
| — | $D_l(t)$ | **New:** Frobenius norm of layer gradients squared |
| — | $E_l^{\text{diss}}(T)$ | **New:** Cumulative dissipation |
...
Consider continuous-time gradient flow on the loss $\mathcal{L}(\theta)$:
...
For a neural network with parameters partitioned by layer $\theta = (W_1, \ldots, W_L)$:

$$\|\nabla_\theta \mathcal{L}\|^2 = \sum_{l=1}^L \|\nabla_{W_l} \mathcal{L}\|_F^2$$

Define the **layer-wise optimization dissipation proxy**:

$$\boxed{D_l(t) = \|\nabla_{W_l} \mathcal{L}(t)\|_F^2}$$

And **total dissipation**:

$$D(t) = \sum_{l=1}^L D_l(t)$$

### Cumulative Dissipation

For discrete time with step size $\Delta t$:

$$\boxed{E_l^{\text{diss}}(T) = \sum_{t=1}^T D_l(t) \cdot \Delta t}$$

For constant learning rate $\eta$, this approximates the integral:

$$E_l^{\text{diss}}(T) \approx \int_0^T D_l(s) \, ds$$
...
- $\mathcal{L}(\theta)$ acts as a **potential energy**
- $-d\mathcal{L}/dt = \|\nabla \mathcal{L}\|^2$ is the **dissipation rate**
...
1. **Lyapunov function:** $\mathcal{L}(\theta)$ is a Lyapunov function for gradient flow; it monotonically decreases along trajectories.

2. **Energy interpretation:** The quantity $\|\nabla \mathcal{L}\|^2$ measures how rapidly the system dissipates potential energy.
...
For discrete gradient descent with learning rate $\eta$:

$$\theta_{t+1} = \theta_t - \eta \nabla \mathcal{L}(\theta_t)$$

The loss change is:

$$\Delta \mathcal{L} = \mathcal{L}(\theta_{t+1}) - \mathcal{L}(\theta_t)$$

Taylor expansion:

$$\Delta \mathcal{L} \approx -\eta \|\nabla \mathcal{L}\|^2 + \frac{\eta^2}{2} \nabla \mathcal{L}^T H \nabla \mathcal{L} + O(\eta^3)$$

**Key insight:** The dissipation identity $-\Delta \mathcal{L} \approx \eta \|\nabla \mathcal{L}\|^2$ only holds when:
- $\eta$ is small (first-order approximation valid)
- Curvature term $\frac{\eta^2}{2} \nabla \mathcal{L}^T H \nabla \mathcal{L}$ is negligible

**Edge of Stability (Cohen et al., 2021):** When the maximum Hessian eigenvalue $\lambda_{\max}(H) \approx 2/\eta$, the quadratic term is no longer negligible, and loss can increase temporarily. This is commonly observed in neural network training.

**Implication:** Our dissipation proxy $D_l(t)$ may not equal $-\Delta \mathcal{L}/\eta$ at edge-of-stability regimes. We should record both $D_l(t)$ and $-\Delta \mathcal{L}(t)$ to detect discrepancies.

### 8.2 SGD with Momentum

With momentum coefficient $\beta$:
...
$$\Delta \theta = -\eta(\beta v_t + \nabla \mathcal{L})$$

**Problem:** Parameter displacement $\Delta \theta$ includes momentum history, not just current gradient.

**Solution:** Record both:
- $D_l(t) = \|\nabla_{W_l} \mathcal{L}(t)\|_F^2$ (gradient-based)
- $\|\Delta W_l(t)\|_F$ (actual displacement)
...
$$\Delta \theta \propto \frac{m_t}{\sqrt{v_t} + \epsilon}$$

**Problems:**
1. Gradient magnitudes are normalized by $\sqrt{v_t}$, breaking direct connection to dissipation
...
2. For Adam experiments, treat $D_l(t) = \|\nabla_{W_l} \mathcal{L}\|_F^2$ as an **activity measure** rather than true dissipation
...
$$\nabla \mathcal{L}_{\text{batch}} = \nabla \mathcal{L}_{\text{true}} + \xi$$

where $\xi$ is gradient noise with variance $\sigma^2/B$ (batch size $B$).
...
|----------|--------|------------|
| Training loss | $\mathcal{L}_{\text{train}}(t)$ | Loss on training set |
| Validation loss | $\mathcal{L}_{\text{val}}(t)$ | Loss on held-out validation set |
| Training accuracy | $\text{Acc}_{\text{train}}(t)$ | Proportion correct on training set |
| Validation accuracy | $\text{Acc}_{\text{val}}(t)$ | Proportion correct on validation set |
| Epoch number | $t$ | Training epoch |
| Wall-clock time | $T_{\text{wall}}(t)$ | Elapsed time |

### 9.2 Optimization Variables (Every Layer, Every Checkpoint)

| Variable | Symbol | Definition |
|----------|--------|------------|
| Instantaneous dissipation | $D_l(t)$ | $\|\nabla_{W_l}\mathcal{L}(t)\|_F^2$ |
| Parameter displacement | $\|\Delta W_l(t)\|_F$ | $\|W_l(t+1) - W_l(t)\|_F$ |
| Relative displacement | $R_{W,l}(t)$ | $\frac{\|\Delta W_l(t)\|_F}{\|W_l(t)\|_F + \epsilon}$ |
| Cumulative dissipation | $E_l^{\text{diss}}(t)$ | $\sum_{s=1}^t D_l(s) \Delta t$ |
| Parameter norm | $\|W_l(t)\|_F$ | Frobenius norm of layer weights |
| Gradient direction | $\hat{g}_l(t)$ | $\nabla_{W_l}\mathcal{L}/\|\nabla_{W_l}\mathcal{L}\|_F$ (optional, for alignment) |
...
| Layer curvature (average) | $K_l(t)$ | $\frac{1}{C}\sum_c K_l^c(t)$ |
| Curvature variance | $\text{Var}[K_l](t)$ | Variance across classes or samples |
| Curvature change | $\Delta K_l(t)$ | $K_l(t+1) - K_l(t)$ |
| Smoothing quantity | $S_l(t)$ | $-\Delta|K_l(t)|$ (if curvature magnitude meaningful) |

Using Kaufman-Azencot CAML (for validation):

| Variable | Symbol | Definition |
|----------|--------|------------|
| MAPC per layer | $\text{MAPC}_l(t)$ | Mean absolute principal curvature at layer $l$ |
| MAPC gap | $\Delta\text{MAPC}(t)$ | Gap between last two layers |
| Intrinsic dimension | $\text{ID}_l(t)$ | TwoNN estimate of intrinsic dimension |

### 9.4 Robustness Variables (Selected Checkpoints)

| Variable | Symbol | Definition |
|----------|--------|------------|
| Clean accuracy | $R_{\text{clean}}(t)$ | Test accuracy on clean data |
| Noise robustness | $R_{\sigma}(t)$ | Accuracy under Gaussian noise (std=$\sigma$) |
| Corruption robustness | $R_{\text{corr}}(t)$ | Accuracy under controlled corruptions |
| FGSM robustness | $R_{\text{FGSM}}(t)$ | Accuracy under FGSM attack ($\epsilon$-bounded) |
| PGD robustness | $R_{\text{PGD}}(t)$ | Accuracy under PGD attack |

### 9.5 Jacobian/Metric Variables (Small Experiments Only)

| Variable | Symbol | Definition |
|----------|--------|------------|
| Layer Jacobian | $J_l(x,t)$ | $\frac{\partial h_l}{\partial h_{l-1}}$ |
| Induced metric | $g_l(x,t)$ | $J_l^T J_l$ |
| Metric change | $\Delta g_l(t)$ | Change in induced metric |
| Jacobian singular values | $\sigma_k(J_l)$ | Singular values of Jacobian |
...
- Checkpoint when $|\Delta \mathcal{L}| > \tau$ or every $k$ epochs (whichever first)
...
$$D_l(t) \to \Delta K_l(t+\tau)$$

Analysis:
1. **Lagged cross-correlation:** Compute $\text{Corr}(D_l(t), \Delta K_l(t+\tau))$ for $\tau \in \{0, 1, 2, 5, 10\}$ epochs
2. **Lagged regression:** $\Delta K_l(t+\tau) = \beta_0 + \beta_1 D_l(t) + \epsilon$
...
$$\Delta K_{l,r,t} = \beta_0 + \beta_1 D_{l,r,t} + u_r + v_{l,r} + \epsilon_{l,r,t}$$
...
| Lagged | $(D_l(t), \Delta K_l(t+\tau))$ | Cross-correlation |
| Cumulative | $(E_l^{\text{diss}}(T), K_l(T))$ | Regression |
| Robustness | $(K_l(T), R(T))$ | Correlation, regression |
| Incremental | $(D_l(t), K_l(t)) \to R(T)$ | Multiple regression, $R^2$ decomposition |
...
- Bonferroni: $\alpha_{\text{adj}} = 0.05 / (4 \times 5 \times 3) = 0.0008$
...
**Control:**
- Regress out training epoch: $\Delta K_l(t) \sim D_l(t) + t$
...
**Problem:** $D_l(t) = \|\nabla_{W_l}\mathcal{L}\|_F^2$ may correlate with curvature simply because larger gradients → larger representation changes.

**Control:**
- Separate gradient magnitude from gradient direction
- Analyze $\Delta K_l / D_l$ (curvature change per unit dissipation)
...
**Control:**
- Normalize $D_l$ by number of parameters: $\tilde{D}_l = D_l / |W_l|$
...
- Panel C: $\Delta K_l(t)$ heatmap (layer × epoch, color = curvature change)
...
- Panel A: Cross-correlation $\text{Corr}(D_l(t), \Delta K_l(t+\tau))$ vs $\tau$ for each layer
- Panel B: Optimal lag $\tau^*$ by layer
...
- Panel B: Cumulative dissipation $E_l^{\text{diss}}(T)$ vs robustness
- Panel C: $R^2$ decomposition: curvature-only, dissipation-only, combined
...
- The edge-of-stability phenomenon shows that $\|\nabla \mathcal{L}\|^2$ dynamics are coupled to second-order loss properties.
...
The hypothesis proposes: $D_l(t) \to \Delta K_l(t+\tau)$
...
- Record both $D_l$ and $-\Delta \mathcal{L}/\eta$ to detect discrepancies
...
The proposed smoothing quantity $S_l(t) = -\Delta|K_l(t)|$ assumes curvature magnitude is meaningful. This is valid for the weighted-angle definition (angles are non-negative and interpretable).

**Issue 3: Layer-Wise vs. Class-Wise Curvature**

Sekmen-Bilgin compute curvature **per class** and average. This should be preserved:
$$K_l(t) = \frac{1}{C}\sum_{c=1}^C K_l^c(t)$$
